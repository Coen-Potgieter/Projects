...
class ConvLayer:
    ...
    def backward_prop(self):
        ...
        self.dk /= self.num_examples
        self.db /= self.num_examples
...
def init_params(input_shape: tuple, kernel_size: int, output_depth: int):
    ...
    k = np.random.uniform(low=-0.01, high=0.01, size=kernel_shape)
    ...
    *full explanation is in notes*
    ...
    for depth_idx in range(dz.shape[2]):
        for channel in range(x.shape[2]):
            dk[:, :, channel, depth_idx] = my_convolute(x[:, :, channel],
                                                      dz[:, :, depth_idx],
                                                      mode="valid")
    ...
    for channel in range(x.shape[2]):
        for depth_idx in range(dz.shape[2]):
            ...
            dx[:, :, channel] += my_convolute(dz[:, :, depth_idx],
                                            k180, mode="full")
