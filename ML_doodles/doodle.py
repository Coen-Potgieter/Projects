...
import mlp_brain
import normalize
from data_size_checker import ITEMS
...
def main():
    ...
    def display_perc(perc):
        ...
        perc = [[ITEMS[elem[0]], float('%.2f' % elem[1])] for elem in perc]

        perc.sort(reverse=True, key=myFunc)

        x = 670
        running_y = 30
        stats = [[i, Text(perc[i])] for i in range(len(ITEMS))]
    ...
    def display_text():
        ...
        WIN.blit(instr_text, (WIDTH//2 + 20 , HEIGHT - 90))

        instr_text = INSTR_FONT.render(
                    f'[S] to Predict', True, YELLOW)
        WIN.blit(instr_text, (WIDTH//2 + 50 , HEIGHT - 50))
    # ------------------------------------------------------- #
